package modules.load
{
	...
	public class LoadManager extends ModulesManager
	{
		...
		private function onLoadResource(event:LoadEvent):void
		{
			...
			//添加资源组加载完成回调函数
			var loadingUrls:Array = urls.concat();
			urlsFuncsDic[loadingUrls] = {func: allItemsLoaded, param: allItemsLoadedParam};
		...
		private function singleLoaded(loadingItem:LoadingItem):void
		{
			...
			//检查资源组加载完成，并执行回调函数
			for (var loadingUrls:Array in urlsFuncsDic)
			{
				var index:int = loadingUrls.indexOf(loadedUrl);
				if (index != -1)
				{
				...
				{
					var allItemsLoadedObj:Object = urlsFuncsDic[loadingUrls];
					...
					urlsFuncsDic[loadingUrls] = null;
					delete urlsFuncsDic[loadingUrls];
				}
			}
		}
